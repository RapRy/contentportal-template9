...
const Main = () => {
  ...
  return (
    <div className="bg-white pt-16 sm:pt-28 max-w-screen-lg mx-auto">
      {contents.length > 0 ? (
            ...
            <div className="relative pt-5" key={content._id}>
              <Link to={`/Content/${content._id}`}>
                <div className="contThumb absolute left-1/2 top-0 transform -translate-x-1/2 shadow-contentShadow rounded-lg cursor-pointer">
                  <img
                    className="contThumb object-cover rounded-lg"
              ...
              <div className="px-3 pb-3 pt-16 5 h-full bg-white rounded-lg shadow-contentShadow">
                <button className="font-rubik font-normal text-white bg-lightRed py-1 px-2.5 rounded-md text-sm block mb-2.5 mx-auto">
                  Download
                </button>
                <p className="font-rubik font-medium text-sm text-center">
                  {content.name}
                </p>
              </div>
            </div>
        ...
        <h1 className="font-rubik text-black text-center font-bold text-xl">
          {search !== undefined
            ...
            : `No contents available for ${subcat}`}
        </h1>
      )}
    </div>
  );
