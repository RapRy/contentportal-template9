                    ...
                    newCats.map((cat, i) => (
                        <div key={cat._id} className="sm:inline-block sm:border-l-2 sm:first:border-l-0 sm:border-gray-1">
                            <div className="block py-2.5 sm:py-1 px-5 relative top-0 left-0 z-20">
                                <NavLink to={`/Category/${cat.catName}/${cat.subCategories[0].subCatName}`} onClick={() => showSubCats(i)}>
                                    <button className={`font-rubik bg-none cursor-pointer font-bold text-xl ${cat.active === false ? "text-black" : "text-gray-2"}`}>{cat.catName}</button>
                                </NavLink>
                            </div>
                            { isMobile && <Subcategories cat={cat} setActiveSubcat={setActiveSubcat} activeSubcat={activeSubcat} setNavToggle={setNavToggle} isMobile={isMobile} /> }
                        </div>
                    ))
                }
            </div>
            { !isMobile && !_.isEmpty(activeCat) &&  
                <Subcategories cat={activeCat} setActiveSubcat={setActiveSubcat} activeSubcat={activeSubcat} setNavToggle={setNavToggle} isMobile={isMobile} />
            }
        </>
    )
}
